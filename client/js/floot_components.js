...
function NewFlootEntry(selectedUser, actions) {
    ...
    function postFloot() {
      actions.createFloot(textbox.value, selectedUser);
...
function Floot(flootInfo, selectedUser, showDelete, actions) {
    ...
    function handleCardClick() {
      actions.openFlootInModal(selectedUser, actions, flootInfo);
    ...
    function deleteFloot() {
      actions.deleteFloot(flootInfo.id, selectedUser);
    ...
    function toggleLike(e) {
        ...
        e.stopPropagation();
        actions.likeUnlikeFloots(flootInfo.id, selectedUser, actions);
...
function LikeCommentCount(flootInfo, selectedUser, onLike) {
    ...
    let numLikes = Object.keys(flootInfo.liked_by).length;
    console.log(flootInfo.linked_by);
    if (flootInfo.liked_by.indexOf(selectedUser) !== -1) {
      let likeBtn = LikeCount(numLikes, true);
      likeBtn.addEventListener("click", onLike);
      container.appendChild(likeBtn);
      console.log("hi all");
    }
    else{
      let likeBtn = LikeCount(numLikes, false);
      likeBtn.addEventListener("click", onLike);
      container.appendChild(likeBtn);
      console.log("ninin");
    }
