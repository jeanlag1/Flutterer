...
function Flutterer() {
  ...
  req.setSuccessHandler(function(response){
    ...
    let actions = {
      ...
      likeUnlikeFloots : function(floot_id, user, actions){
        let requ = AsyncRequest("/api/floots/" + floot_id);
        requ.setSuccessHandler(function(response){
          let floot = JSON.parse(response.getPayload());
          //unlike
          if (floot.liked_by.indexOf(user) !== -1){
            let postReq = AsyncRequest("/api/floots/" + floot_id + "/unlike");
            postReq.setMethod("POST");
            postReq.setPayload(JSON.stringify({
              username : user,
            }));
            postReq.setSuccessHandler(function(){
              GetApiFlootsAndReRender(user, actions, null);
              console.log("lolo");
            });
            postReq.send();
          }
          else{
            let postReq = AsyncRequest("/api/floots/" + floot_id + "/like");
            postReq.setMethod("POST");
            postReq.setPayload(JSON.stringify({
              username : user,
            }));
      ...
      // unlikeFloot : function (floot_id, username, actions){
      //
      // },
      // likeFloot : function(floot_id, username, actions){
      //
      // },
    };
